...
class Asset {
	 ...
	 */
	public function css($asset_name, $module_name = NULL, $attributes = array(), $inline = FALSE)
	{

		if($inline === TRUE) {
			return '<style>' . $asset_name . '</style>';
		}
	...
	private function _other_asset_location($asset_name, $module_name = NULL, $asset_type = NULL, $location_type = 'url')
	{
		...
		if (strpos($asset_name, '//') !== FALSE)
		// parce que c'est mieux d'écrire //url plutôt que http://url ou https://url, cela laisse le choix au navigateur
		{
			return $asset_name;
		}
		...
		else
		{
			...
			$type_path = ($asset_type == '') ? '' : $asset_type ;
			...
			if ($module_name)
			{
				...
				//{
					...
					//{
						...
						$asset_location .=  $type_path . '/modules/' . $module_name . '/' ;
					//	break;
				//	}
				//}
			}
			else {
				$asset_location .= $type_path . '/';
			}
